...
const initialMembers = [
    { id: 1, name: 'Ahmad Subagja', gender: 'Male', court: 'Court A', phone: '123-456-7890', status: 'Paid', joinDate: '2025-01-15' },
    { id: 2, name: 'Siti Aminah', gender: 'Female', court: 'Court B', phone: '098-765-4321', status: 'Unpaid', joinDate: '2025-02-20' },
    { id: 3, name: 'Budi Santoso', gender: 'Male', court: 'Court A', phone: '111-222-3333', status: 'Paid', joinDate: '2025-03-10' },
    { id: 4, name: 'Dewi Lestari', gender: 'Female', court: 'Court C', phone: '444-555-6666', status: 'Paid', joinDate: '2024-05-01' },
    { id: 5, name: 'Eko Prasetyo', gender: 'Male', court: 'Court B', phone: '777-888-9999', status: 'Paid', joinDate: '2025-04-12' },
    { id: 6, name: 'Fitriani', gender: 'Female', court: 'Court A', phone: '101-112-131', status: 'Unpaid', joinDate: '2025-02-18' },
    { id: 7, name: 'Gatot Wibowo', gender: 'Male', court: 'Court B', phone: '415-161-718', status: 'Paid', joinDate: '2025-03-25' },
    { id: 8, name: 'Hasanah', gender: 'Female', court: 'Court A', phone: '910-111-213', status: 'Paid', joinDate: '2025-01-30' },
];
const initialBookings = [
    { id: 1, courtName: 'Court A', location: 'City Center', startDate: '2025-06-01', endDate: '2025-08-31', cost: 5000000, maxMembers: 4 },
    { id: 2, courtName: 'Court B', location: 'North Suburb', startDate: '2025-07-01', endDate: '2025-09-30', cost: 4500000, maxMembers: 6 },
    { id: 3, courtName: 'Court C', location: 'West End', startDate: '2024-01-01', endDate: '2024-05-31', cost: 6000000, maxMembers: 8 }, // This booking is closed
];
const initialCashflow = [
    { id: 1, type: 'Income', description: 'Membership Fee - Ahmad S.', amount: 1250000, date: '2025-06-05' },
    { id: 2, type: 'Expense', description: 'Shuttlecock Purchase', amount: 250000, date: '2025-06-10' },
    { id: 3, type: 'Income', description: 'Court C Final Payment', amount: 6000000, date: '2024-05-28' },
];
...
// --- Core App Components ---
const Sidebar = ({ page, setPage, handleLogout, isOpen, setIsOpen }) => (
...
const Dashboard = ({ members, bookings, transactions, tournaments }) => {
    ...
    const recentActivities = useMemo(() => {
        const recentMembers = members.slice(-2).map(m => ({ type: 'New Member', text: `${m.name} has joined.`, date: m.joinDate, icon: <UserPlus className="text-blue-500"/> }));
        const recentCashflow = transactions.slice(-2).map(t => ({ type: t.type, text: `${t.description}`, date: t.date, icon: t.type === 'Income' ? <DollarSign className="text-green-500"/> : <DollarSign className="text-orange-500"/> }));
        const recentTournaments = tournaments.slice(-1).map(t => ({ type: 'New Tournament', text: `${t.name} has been created.`, date: t.createdAt, icon: <Trophy className="text-indigo-500"/> }));
...
const Membership = ({ members, setMembers, bookings }) => {
    ...
    const activeMembers = useMemo(() => {
        const activeCourtNames = bookings.filter(b => getBookingStatus(b) === 'Open').map(b => b.courtName);
        return members.filter(m => activeCourtNames.includes(m.court));
    }, [members, bookings]);
    ...
    const handleFormSubmit = (e) => {
        ...
        const courtBooking = bookings.find(b => b.courtName === memberData.court);
        const membersInCourt = members.filter(m => m.court === memberData.court);
        if (!editingMember && courtBooking && membersInCourt.length >= courtBooking.maxMembers) {
            setError(`Cannot add member. ${memberData.court} is full (${courtBooking.maxMembers} members max).`);
            return;
        }
        if (editingMember) {
            setMembers(members.map(m => m.id === editingMember.id ? { ...m, ...memberData } : m));
        } else {
            setMembers([{ id: Date.now(), ...memberData, joinDate: new Date().toISOString().split('T')[0]}, ...members]);
        }
        setShowModal(false); setEditingMember(null);
    };
    const handleDelete = () => { setMembers(members.filter(m => m.id !== deletingMember.id)); setDeletingMember(null); };
    const membersWithCost = useMemo(() => activeMembers.map(member => {
            const courtBooking = bookings.find(b => b.courtName === member.court);
            if (!courtBooking) return { ...member, individualCost: 0 };
            const membersInCourt = activeMembers.filter(m => m.court === member.court).length;
...
const Booking = ({ bookings, setBookings }) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        const formData = new FormData(e.target);
        const bookingData = { ...Object.fromEntries(formData.entries()), cost: parseFloat(formData.get('cost')), maxMembers: parseInt(formData.get('maxMembers')) };
        if (editingBooking) { setBookings(bookings.map(b => b.id === editingBooking.id ? { ...b, ...bookingData } : b)); } 
        else { setBookings([...bookings, { id: Date.now(), ...bookingData }]); }
        setShowModal(false); setEditingBooking(null);
    };
    const handleDelete = () => { setBookings(bookings.filter(b => b.id !== deletingBooking.id)); setDeletingBooking(null); };
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {bookings.map(booking => { const status = getBookingStatus(booking); return (<div key={booking.id} className="bg-white p-5 rounded-xl border border-gray-200 group"><div className="flex justify-between items-start"><h3 className="text-lg font-bold text-gray-800">{booking.courtName}</h3><div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => { setEditingBooking(booking); setShowModal(true); }} className="text-gray-500 hover:text-indigo-600 p-1.5 rounded-md hover:bg-gray-100"><Edit size={16}/></button><button onClick={() => setDeletingBooking(booking)} className="text-gray-500 hover:text-red-600 p-1.5 rounded-md hover:bg-red-50"><Trash2 size={16}/></button></div></div><p className="text-sm text-gray-500 mb-4">{booking.location}</p><div className="mt-4 pt-4 border-t border-gray-100 text-sm space-y-2"><p className="flex items-center justify-between"><span className="font-semibold text-gray-600">Period:</span> <span className="text-gray-800 font-medium">{formatDate(booking.startDate)} - {formatDate(booking.endDate)}</span></p><p className="flex items-center justify-between"><span className="font-semibold text-gray-600">Total Cost:</span> <span className="text-gray-800 font-medium">{formatCurrency(booking.cost)}</span></p><p className="flex items-center justify-between"><span className="font-semibold text-gray-600">Max Members:</span> <span className="text-gray-800 font-medium flex items-center gap-1"><Users size={14}/>{booking.maxMembers}</span></p><p className="flex items-center justify-between"><span className="font-semibold text-gray-600">Status:</span><span className={`px-2.5 py-1 rounded-full font-medium text-xs ${status === 'Open' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{status}</span></p></div></div>)})}
            </div>
            {showModal && <Modal onClose={() => { setShowModal(false); setEditingBooking(null); }} title={editingBooking ? 'Edit Booking' : 'Add New Booking'}><form onSubmit={handleFormSubmit} className="space-y-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-700 mb-1">Court Name</label><input type="text" name="courtName" defaultValue={editingBooking?.courtName} className="w-full p-2.5 border border-gray-300 rounded-lg" required /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Location</label><input type="text" name="location" defaultValue={editingBooking?.location} className="w-full p-2.5 border border-gray-300 rounded-lg" required /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Start Date</label><input type="date" name="startDate" defaultValue={editingBooking?.startDate} className="w-full p-2.5 border border-gray-300 rounded-lg" required /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">End Date</label><input type="date" name="endDate" defaultValue={editingBooking?.endDate} className="w-full p-2.5 border border-gray-300 rounded-lg" required /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Total Cost (Rp)</label><input type="number" name="cost" defaultValue={editingBooking?.cost} className="w-full p-2.5 border border-gray-300 rounded-lg" required /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Max Members</label><input type="number" name="maxMembers" defaultValue={editingBooking?.maxMembers} className="w-full p-2.5 border border-gray-300 rounded-lg" required /></div></div><div className="flex justify-end gap-3 pt-4"><button type="button" onClick={() => { setShowModal(false); setEditingBooking(null); }} className="bg-gray-100 text-gray-800 px-5 py-2.5 rounded-lg font-semibold text-sm">Cancel</button><button type="submit" className="bg-indigo-600 text-white px-5 py-2.5 rounded-lg font-semibold text-sm">Save</button></div></form></Modal>}
            {deletingBooking && <ConfirmationModal title="Delete Booking" message={`Are you sure you want to delete the booking for ${deletingBooking.courtName}?`} onConfirm={handleDelete} onCancel={() => setDeletingBooking(null)} />}
        </div>
    );
};
const Cashflow = ({ transactions, setTransactions }) => {
    ...
    const handleFormSubmit = (e) => {
        ...
        if (editingTransaction) { setTransactions(transactions.map(t => t.id === editingTransaction.id ? { ...t, ...transactionData } : t)); } 
        else { setTransactions([{ id: Date.now(), ...transactionData }, ...transactions].sort((a,b) => new Date(b.date) - new Date(a.date))); }
        setShowModal(false); setEditingTransaction(null);
    };
    const handleDelete = () => { setTransactions(transactions.filter(t => t.id !== deletingTransaction.id)); setDeletingTransaction(null); };
    return (
        <div className="p-4 sm:p-6 md:p-8">
            ...
            {deletingTransaction && <ConfirmationModal title="Delete Transaction" message={`Are you sure you want to delete this transaction?`} onConfirm={handleDelete} onCancel={() => setDeletingTransaction(null)} />}
        </div>
    );
};
const Tournament = ({ tournaments, setTournaments, members }) => {
    // Note: The logic inside Tournament remains mostly the same, with minor adjustments.
    const [showCreateModal, setShowCreateModal] = useState(false);
    const [showEditModal, setShowEditModal] = useState(null);
    const [deletingTournament, setDeletingTournament] = useState(null);
    const [viewingTournament, setViewingTournament] = useState(null);

    const handleCreate = (newTournament) => {
        setTournaments([...tournaments, { id: Date.now(), ...newTournament, createdAt: new Date().toISOString() }]);
        setShowCreateModal(false);
    };

    const handleUpdate = (updatedTournament) => {
    ...
    if (viewingTournament) {
        return <TournamentBracket tournament={viewingTournament} setTournament={(t) => { setTournaments(tournaments.map(ot => ot.id === t.id ? t : ot)); setViewingTournament(t); }} onBack={() => setViewingTournament(null)} />;
    }
...
const TournamentBracket = ({ tournament, setTournament, onBack }) => {
    ...
    useEffect(() => {
        ...
        if (winners.length === lastRound.length && winners.length > 1 && winners.length % 2 === 0) {
            const nextRound = [];
            for (let i = 0; i < winners.length; i += 2) { nextRound.push({ matchId: `r${bracket.length}-m${i/2}`, team1: winners[i], team2: winners[i+1], winner: null }); }
    ...
    const setWinner = (roundIndex, matchIndex, winner) => {
        ...
        if (roundIndex === newBracket.length -1 && newBracket[newBracket.length-1].every(m => m.winner)) { updatedTournament.status = 'Completed'; } 
        else { updatedTournament.status = 'In Progress'; }
                                ...
                                {[match.team1, match.team2].map((team, teamIndex) => team && (
                                    <div key={teamIndex} onClick={() => team.name !== 'BYE' && !match.winner && setWinner(roundIndex, matchIndex, team)} className={`flex justify-between items-center p-2 rounded-md ${!match.winner && team.name !== 'BYE' ? 'cursor-pointer hover:bg-indigo-50' : ''} ${match.winner?.id === team.id ? 'bg-green-100' : ''}`}>
                                        <div>
                                            ...
                                            {team.players.length > 1 && <div className="text-xs text-gray-500 mt-1">{team.players.map(p => p.name).join(' & ')}</div>}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                ...
                <div className="bg-white p-6 rounded-xl border-4 border-indigo-500 text-center flex flex-col">
                 ...
                 <div className="bg-white p-6 rounded-xl border-4 border-gray-300 text-center flex flex-col">
                    <input type="text" value={name2} onChange={(e) => setName2(e.target.value)} className="text-3xl font-bold text-gray-800 text-center bg-transparent focus:outline-none focus:bg-gray-100 rounded-lg py-2 mb-4" />
...
export default function App() {
    ...
    const [transactions, setTransactions] = useState(initialCashflow);
    ...
    return (
        <div className="flex bg-gray-50 min-h-screen font-sans">
            <Sidebar page={page} setPage={setPage} handleLogout={handleLogout} isOpen={isSidebarOpen} setIsOpen={setIsSidebarOpen} />
